...
//     const response = await fetch(`http://promech-backend.addictaco.com/api/herosections/?populate=image`,{
...
// 	/////////////////////////////////////////////////////////////////////////////////////////////////////////
//     const responsetracks = await fetch(`http://promech-backend.addictaco.com/api/tracks/?pagination[limit]=2&populate=mainImgSrc&populate=courses`,{
...
// 	const responsecourses = await fetch(`http://promech-backend.addictaco.com/api/courses/?pagination[limit]=3&populate=imgSrc&populate=chapters&populate=userimage`, {
//         next: {
//             revalidate: 0
//         }
//     });
...
// 	/////////////////////////////////////////////////////////////////////////////////////
// 	const responseSection = await fetch(`http://promech-backend.addictaco.com/api/sectionsinhomes/?populate=imagesection`, {
//         next: {
//             revalidate: 0
//         }
//     });
...
// 							<Image alt='ellipse' 
// 							src={data&&data?.data[0]&&data?.data[0]?.attributes&&data?.data[0]?.attributes?.image?`http://promech-backend.addictaco.com${data?.data[0]?.attributes?.image?.data.attributes.url}`:'/home/ellipse.jpg'}
...
// 								src={el?.attributes?.imagesection?.data?.attributes?.url?`http://promech-backend.addictaco.com${el?.attributes?.imagesection?.data?.attributes?.url}`:'/home/solutions.jpg'}
...
// 							>
// 								<Image
// 									src={track&&track?.attributes&&track?.attributes?.mainImgSrc&&track?.attributes?.mainImgSrc?.data&&track?.attributes?.mainImgSrc&&track?.attributes?.mainImgSrc?.data?.attributes?.url?`http://promech-backend.addictaco.com${track?.attributes?.mainImgSrc&&track?.attributes?.mainImgSrc?.data?.attributes?.url}`:''}
    ...
    <>
          ...
          <Typography lineHeight={2} fontSize='18px' maxWidth={600}>
            Join us to get empowered and lead the next generation of sustainable manufacturing.
							...
							<Button style={{ scrollBehavior: 'smooth' }}  variant='contained' color='inherit' size='large'
								endIcon={<EastIcon color='success' />}
								sx={{ bgcolor: '#FFF', color: 'primary.main', fontWeight: 600, fontSize: '18px', paddingY: 2, paddingX: 4 }}
							>
								  <a     href="#sectionId" style={{ textDecoration: 'none', color: 'inherit' }}>
...
<Image
  ...
  src={
    ...
    data?.data[0]?.attributes?.image?.data?.attributes?.url
      ? `http://promech-backend.addictaco.com${data?.data[0]?.attributes?.image?.data.attributes.url}`
      : '/home/ellipse.jpg'
  }
  width={405}
  height={405}
...
<>
<Box  paddingY={{ xs: 2, sm: 4, md: 6, lg: 8 }} paddingX={{ xs: 1, lg: 0 }} paddingBottom={{ xs: 8, sm: 10, md: 12, lg: 12 }}>
				<Container  disableGutters maxWidth='lg' sx={{ position: 'relative', display: 'flex',  alignItems: 'center', gap: 2 }}>
					<Box component='img' alt='ellipse' src='/home/ellipse2.svg' sx={{ alignSelf: 'flex-end', position: { xs: 'absolute', md: 'block' }, bottom: { xs: -8*8, sm: -8*10, md: -8*10, lg: -8*12 }, left: 0 }} />
					<Stack gap={4} width='100%'>
						<Typography variant='h4' color='text.secondary' lineHeight='55px' fontWeight={700}>
    ...
    wordWrap: "break-word" 
        ...
        sx={{ scrollbehavior:'smooth' }} >
				<Container style={{scrollBehavior:"smooth"}} disableGutters maxWidth='lg' sx={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
					<Stack  alignItems='center' width='100%' gap={2}>
						<Typography  variant='h4' color='primary.main' fontWeight={500} lineHeight='normal'>Explore</Typography>
						<Typography variant='h4' fontWeight={700} lineHeight='normal'>Promech Academy Tracks</Typography>
						<Divider variant='middle' sx={{ bgcolor: 'primary.main', width: 100, opacity: 0.3, marginTop: 2 }} />
